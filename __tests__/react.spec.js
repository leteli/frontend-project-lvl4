...
test.describe('auth', () => {
  ...
  test('successful login', async ({ page }) => {
    await page.locator('text=Ваш ник').type(user.login);
    await page.locator('text=/^Пароль$/').type(user.password);
    await page.locator('button[type="submit"]').click();
...
test.describe('chat', () => {
  ...
  test('profanity filter', async ({ page }) => {
    ...
    await page.keyboard.press('Enter');
    expect(await page.$(`text=${profanityText}`)).toBeNull();
    expect(await page.$('text=you have nice *****')).not.toBeNull();
  });
  ...
  test('rename channel', async ({ page }) => {
    ...
    await expect(await page.$('text=Канал переименован')).not.toBeNull();
  ...
  test('remove channel', async ({ page }) => {
    ...
    await expect(await page.$('text=Канал удалён')).not.toBeNull();
